...
public class Benchmark implements Runnable {
    ...
    public Benchmark(IBenchmark impl, AtomicLong counter, Semaphore semaphore,
            Thread parent) {
        _benchmark = impl;
        _counter = counter;
    ...
    public static void main(String[] argv) throws Exception {
        ...
        Class<? extends IBenchmark> factoryClass = Class.forName(factoryName)
                .asSubclass(IBenchmark.class);
        Constructor<? extends IBenchmark> factoryConstructor = factoryClass
        ...
        try {
            for (int i = 0; i < threadCount; i++) {
                Thread t = new Thread(new Benchmark(
                        factoryConstructor.newInstance(docbase, userName,
                                password), counter, semaphore,
            ...
            for (int iteration = 0; iteration < iterationsCount; iteration++) {
                ...
                System.out.println("Executions per " + sleepTime + "ms: "
                        + (curValue - prevValue) + ", iteration: "
                        + (iteration + 1));
                prevValue = curValue;
            }
            success = true;
